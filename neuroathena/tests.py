...
from neuroathena.views import wagtail_logout_with_sso
...
class WagtailLogoutWithSSOTest(TestCase):
    ...
    @override_settings(HABILITAR_SSO_LOGIN=True)
    @patch('neuroathena.views.SSO_AVAILABLE', True)
    @patch('neuroathena.views.obter_provedor_recente')
    ...
    @override_settings(HABILITAR_SSO_LOGIN=True)
    @patch('neuroathena.views.SSO_AVAILABLE', True)
    @patch('neuroathena.views.pode_fazer_logout_sso', return_value=True)
    @patch('neuroathena.views.logout_sso')
    @patch('neuroathena.views.obter_provedor_recente')
